...
package com.valerivp.bluetoothle;
...
import com.valerivp.bluetoothle.BluetoothLE.BluetoothConnectionListener;
...
final class BluetoothLEint {
  ...
  private abstract class BLEReadOperation<T> extends BLEOperation {
    ...
    public void unsubscribe(final BluetoothGatt gatt) {
      synchronized (pendingOperationsByUuid) {
        if (gatt == null || gatt.setCharacteristicNotification(characteristic, false)) {
  ...
  BluetoothLEint(final BluetoothLE outer, final Activity activity, ComponentContainer container) {
  ...
  void StartScanningForService(String caller, final UUID serviceUuid, final DeviceCallback callback) {
    new BLEAction<Void>(caller) {
      ...
      public Void action() {
        ...
        mBluetoothLeDeviceScanner = btAdapter.getBluetoothLeScanner();
        if (mBluetoothLeDeviceScanner == null) return null;
        ScanSettings settings = new ScanSettings.Builder()
            .setScanMode(ScanSettings.SCAN_MODE_LOW_LATENCY)
            .build();
        List<ScanFilter> filters = new ArrayList<ScanFilter>();
        ...
        if (callback != null) {
            ...
            public void onScanResult(int callbackType, ScanResult result) {
              ...
              if (result != null) {
                ...
                if (callback.foundDevice(device.getName(), device.getAddress())) {
                  ...
                  mBluetoothGatt = device.connectGatt(activity, autoReconnect, mGattCallback);
          ...
          mBluetoothLeDeviceScanner.startScan(filters, settings, scanCallback);
          isScanning = true;
        } else {
          mBluetoothLeDeviceScanner.startScan(filters, settings, mDeviceScanCallback);
        }
  ...
  void RequestMTU(final int bytes) {
    new BLEAction<Void>("RequestMTU") {
      @Override
      public Void action() {
        if (mBluetoothGatt != null) {
          mBluetoothGatt.requestMtu(bytes);
        }
        return null;
      }
    }.run();
...
//		else
//			container.$form().dispatchErrorOccurredEvent(outer, "ReadBytes",
...
//				"BluetoothLE", UNKNOWN_CHAR_NAME);
  ...
  void StartAdvertising(final String inData, final String serviceUuid) {
    new BLEAction<Void>("StartAdvertising"){
      ...
      public Void action() {
        ...
        ParcelUuid pUuid = new ParcelUuid(BLEUtil.bleStringToUuid(serviceUuid));
  ...
  String GetAdvertisementData(String deviceAddress, String serviceUuid) {
    ...
    return "" + Arrays.toString(scannedAdvertisers.get(deviceAddress).getScanRecord()
        .getServiceData().get(new ParcelUuid(BLEUtil.bleStringToUuid(serviceUuid))));
  }
  ...
  List<String>  DeviceList() {
   List<String> deviceInfoList = new ArrayList<String>();
	...
	//if (i != (mLeDevices.size() - 1)) {
	  deviceInfoList.add(mLeDevices.get(i).getAddress() + "\t" + mLeDevices.get(i).getName() + "\t"
  ...
  private void DeviceFound(final BluetoothDevice device) {
    uiThread.post(new Runnable() {
      @Override
      public void run() {
        EventDispatcher.dispatchEvent(outer, "DeviceFound", device.getAddress(), device.getName());
      }
    });
  }
  ...
  YailList GetCharacteristicsForService(String serviceUuid) {
    ...
    try {
      BluetoothGattService service = mBluetoothGatt.getService(BLEUtil.bleStringToUuid(serviceUuid));
      if (service == null) {
    ...
    if (!isServiceRead) {
      ...
      for (BluetoothGattService aMGattService : mGattService) {
        if (aMGattService.getUuid().equals(ser_uuid)) {
			Log.i(LOG_TAG, "Service found: " + aMGattService.getUuid());
		  //BluetoothGattCharacteristic result = aMGattService.getCharacteristic(char_uuid);
		  for(BluetoothGattCharacteristic result : aMGattService.getCharacteristics()){
				Log.i(LOG_TAG, "Characteristic found: " + result.getUuid());
			  if(result.getUuid().equals(char_uuid))
				  return result;
		  }
		  //if(result == null)
			throw new IllegalStateException("Characteristic " + char_uuid + " are not published for service " + ser_uuid + " by the connected device.");
